...
samp1 %>% 
  summarise( Samp_X = mean(area) , Sd_X = sd(area))
```
...
# type your code for the Exercise here, and Run Document
sample_means_small <- list()
for (i in 1:10000) {
   sample_means_small[[i]] <- ames %>% 
                              sample_n(size = 10) %>%
                              summarise(x_bar = mean(area))
}

df <- data.frame(x_bar = unlist(sample_means_small))
df %>% 
       ggplot(aes(x = x_bar)) +
       geom_histogram() 

#This is equivalent to the following code which is at least 50 times FASTER!!!!!
...
sample_means50 <- ames %>%
                    rep_sample_n(size = 50, replace = TRUE, rep = 5000) %>%
                    summarise(price_bar = mean(price))

sample_means50 %>%
  ggplot(aes(price_bar)) + geom_histogram(bins = 30)

sample_means150 <- ames %>%
                    rep_sample_n(size = 150, replace = TRUE, rep = 5000) %>%
                    summarise(price_bar = mean(price))
...
# type your code for this Exercise here, and Run Document
sample_means150 %>% summarise(mean(price_bar))
```
...
# type your code for this Exercise here, and Run Document
sample_means15 <- ames %>%
                    rep_sample_n(size = 15, replace = TRUE, rep = 2000) %>%
                    summarise(price_bar = mean(price))

sample_means15 %>% 
  ggplot(aes(price_bar)) + geom_histogram(bins = 30)

sample_means15 %>% summarise(mean(price_bar))
```
