...
```{r set-seed}
set.seed(23081983)                 # make sure to change the seed
```
...
samp %>%
  ggplot(aes(area)) + geom_histogram(bins=15)

samp %>% 
  summarise(area_bar = mean(area), area_s = sd(area))
...
new_ci <- 
  ames %>%
    rep_sample_n(size = n , replace = TRUE, reps = 50) %>%
      summarise( new_lower_ci = mean(area ) - z_star_99 * sd(area) /sqrt(n) , 
                 new_upper_ci = mean(area ) + z_star_99 * sd(area) /sqrt(n) ) 
```
...
```{}
new_ci <- new_ci %>%
  mutate(capture_mu = ifelse(new_lower_ci < params$mu & new_upper_ci > params$mu, "yes", "no"))
  
new_ci_data <- data.frame(ci_id = c(1:50, 1:50),
                      ci_bounds = c(new_ci$new_lower_ci, new_ci$new_upper_ci),
